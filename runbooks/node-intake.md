...
2. **닿는가, 어디에서?** 경로가 둘인데 두 번째를 잊는다. 전용 BMC 포트가 네트워크에
   ...
   **pass-through 주소는 배선의 증거가 아니다.** 그런 인터페이스가 있는 벤더에서는 그것이
   내부 USB 링크이고 주소도 벤더 기본값이라, 전용 포트가 꽂혀 있든 없든 같은 값이 나온다.
   링크 로컬 대역이라 DHCP 실패로 읽기 쉬운데 그 읽기가 틀렸다. 실제로 그렇게 읽은 사례가
   있다. root가 있으면 `ipmitool lan print 1`이 단서를 하나 더 주지만 두 가지를 조심한다.
   그 명령이 말하는 것은 **BMC가 지금 쓰는 포트**이므로 NIC selection(전용 포트인지 LOM
   공유인지)을 먼저 읽어야 그 답을 전용 포트의 것으로 읽을 수 있고, 답이 나오는 것은
   `IP Address Source`가 DHCP일 때뿐이다. 0.0.0.0이면 임대를 못 받은 것이고(미배선이거나
   DHCP 없는 세그먼트), 주소가 있으면 적어도 임대 시점에는 배선이었다. DHCP가 아니면 이
   명령으로는 알 수 없다. 그때는 **물리 확인이 먼저**다. Redfish `EthernetInterfaces`의
   `LinkStatus`는 pass-through 링크가 있고 플랫폼이 BMC 자격증명을 소유한 호스트에서만
   대안이 되며(인증이 필요하고 스키마상 선택 속성이다), **공용 호스트를 거쳐 BMC 자격증명을
   제시하지 않는다.** 셸을 가진 누구나 같은 링크에 닿는다는 것이 바로 위의 사실이다.
3. **기본 자격증명이 남아 있는가?** 로그인해서 확인하지 않는다. **주소와 SNMP community
   (`lan print`의 `SNMP Community String`)가 공장 기본값이면 비밀번호도 그럴 가능성을 함께
   본다.** 다만 최근 세대는 기기별 고유 비밀번호가 기본이라 문서화된 옛 기본값을 단정하지
   않는다. 플랫폼이 소유한

...
| WireGuard transport net | `10.100.100.0/30` — relay `.1`, sshgw `.2` | `create-sshgw-lxc.sh`; `lightsail/wireguard/wg0.conf.template` (its `AllowedIPs` also carries the guest network and **the api's address**, so a wrong entry breaks relay sync rather than the tunnel); `lightsail/haproxy/haproxy.cfg.template` (`server sshgw 10.100.100.2:22`); `lightsail/nftables/nftables.conf`; `hosts/pve1/interfaces` (the `/30` route and the `.1` FORWARD accept); `PICKLE_RELAY_SOURCE_IP` |

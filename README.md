...
- **배포 스크립트는 체크아웃을 갱신하지 않습니다.** 빌드는 배포 호스트의 체크아웃에 지금 있는
  트리로 돕니다. 배포 전에 그 레포의 HEAD를 올리려는 커밋으로 맞추지 않으면 옛 트리가
  빌드되고 배포는 그대로 성공하므로, readiness 게이트도 롤백도 이 상황을 잡지 못합니다.
  무엇이 올라갔는지는 트리가 아니라 살아 있는 표면으로 확인합니다.

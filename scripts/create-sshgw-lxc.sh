#!/usr/bin/env bash
# Creates the pickle-sshgw LXC (CTID 102, 172.30.1.30) on vmbr1 and provisions
...
# 10.100.100.2/30) to the AWS Lightsail relay, systemd units, and a peer-only
...
# --- 3. Provisioning (every step guarded; transport subnet 10.100.100.0/30) --
# The heredoc is single-quoted: it runs verbatim inside the container. Transport
# constants (10.100.100.2/.1, :22, :2222, :51820) are architectural and fixed.
...
# 10.100.100.2:22 ("address already in use"). This LXC is managed via `pct exec`
...
# then wins the race against the shim's 10.100.100.2:22 bind — the shim dies with
...
PICKLE_SSHGW_API_BASE=http://172.30.1.20:8080
...
SSHGW_PROXYFRONT_LISTEN=10.100.100.2:22
SSHGW_PROXYFRONT_UPSTREAM=127.0.0.1:2222
SSHGW_PROXYFRONT_PEER=10.100.100.1/32
ENVSKEL
...
#PICKLE_TERMINAL_WS_LISTEN=172.30.1.30:8082
#PICKLE_TERMINAL_CONTROL_LISTEN=172.30.1.30:8083
PICKLE_TERMINAL_CONSOLE_ORIGIN=https://pickle.pusan.ac.kr
#PICKLE_TERMINAL_WS_PEER=172.30.1.10
#PICKLE_TERMINAL_CONTROL_PEER=172.30.1.20
...
# address-only so the shim can bind 10.100.100.2 before the tunnel is peered.
umask 077
...
# half of contract condition #2 (the shim's REQUIRE policy + 10.100.100.2 bind
...
table inet sshgw {
    chain input {
        ...
        iifname "wg0" ip saddr 10.100.100.1 tcp dport 22 accept
        ...
        ip saddr 172.30.1.10 tcp dport 8082 accept
        ip saddr 172.30.1.20 tcp dport 8083 accept
    }
    chain forward {
        ...
        iifname "wg0" ip saddr 10.100.100.1 ip daddr 172.29.0.0/16 ct state new accept
        iifname "wg0" ip saddr 10.100.100.1 ip daddr 172.30.1.20 tcp dport 8080 ct state new accept
    }
...
#  - sshgw-proxyfront: the :22 ingress shim. Needs wg0 up to bind 10.100.100.2
...
echo "pickle-sshgw LXC $CTID ready at 172.30.1.30 (services enabled, start after deploy)."

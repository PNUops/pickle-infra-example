...
# 3b-1. pve1 hostname mapping — the Proxmox client pins the PVE root CA and the
# pveproxy cert's SANs cover the hostname (pve1), not the vmbr1 bridge IP, so
# the API must be addressed as https://pve1:8006 (nodes.api_host, api V10).
# Outside the PVE-managed block, so pct config rewrites leave it alone.
if ! grep -q '^172\.30\.0\.1 pve1$' /etc/hosts; then
  printf '\n# pve1 Proxmox API via vmbr1 (cert SAN is the hostname)\n172.30.0.1 pve1\n' >> /etc/hosts
  echo "added pve1 hosts mapping"
fi

# 3b-2. Route to the relay WG transport net via the sshgw LXC. The api answers
# relay sync requests (source 10.100.100.1); without this route the replies
# follow the default gateway (pve1) toward the campus uplink and die. Same-L2
# next hop on vmbr1, so the leg never crosses pve1 FORWARD. The container's
...
[ "$IFACE" = eth0 ] || exit 0
ip route replace 10.100.100.0/30 via 172.30.1.30 dev eth0
ROUTE
  chmod 755 /etc/network/if-up.d/pickle-relay-route
  echo "installed if-up.d relay transport route (10.100.100.0/30 via 172.30.1.30)"
fi
...
#PICKLE_TERMINAL_BRIDGE_URL=http://172.30.1.30:8083
ENVSKEL

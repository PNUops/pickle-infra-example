...
# quotes and $ characters that the second shell `su -c` spawns would eat.
pgq() {
  local out
...
parse_doc() { # file
  ...
  [ -n "$DOC_BODY" ] || { echo "  $f: the document body is empty" >&2; return 1; }
}
...
for f in "${FILES[@]}"; do
  ...
  db_title=$(pgq "select title from terms_versions
                   where doc_type = '$dt' and version = $DOC_VERSION;")
  if [ "$file_digest" = "$db_digest" ] && [ "$db_title" = "$DOC_TITLE" ]; then

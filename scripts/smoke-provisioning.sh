...
#
# Run on the pve1 HOST as root — only the host holds a vmbr2 address
# (172.29.0.1) and can reach guest VMs on :22; LXC 101 cannot. Do NOT run
...
for cmd in curl jq nc qm pct; do
  command -v "$cmd" >/dev/null 2>&1 || {
    echo "missing required command: $cmd (this script must run on the pve1 host as root)"
    exit 2
  }
done

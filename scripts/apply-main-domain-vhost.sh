...
pre_opus=$(curl -sk -o /dev/null -w '%{http_code}' --resolve opus.pusan.ac.kr:443:172.30.1.10 https://opus.pusan.ac.kr/ || true)
...
check "new  :443 (trusted chain)" 200 --resolve "$DOMAIN:443:172.30.1.10" "https://$DOMAIN/"
check "new  /api" 200 --resolve "$DOMAIN:443:172.30.1.10" "https://$DOMAIN/api/v1/meta/status"
...
check "new  /api/v1/auth (limited location live)" 405 --resolve "$DOMAIN:443:172.30.1.10" "https://$DOMAIN/api/v1/auth/login"
# An unknown name must no longer complete a handshake at all.
if curl -sk -o /dev/null --max-time 10 --resolve "unknown.invalid:443:172.30.1.10" https://unknown.invalid/ 2>/dev/null; then
...
for _ in $(seq 1 60); do
  c=$(curl -s -o /dev/null -w '%{http_code}' --resolve "$DOMAIN:443:172.30.1.10" "https://$DOMAIN/api/v1/meta/status" || true)

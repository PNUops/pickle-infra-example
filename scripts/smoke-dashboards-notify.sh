...
# Dashboards-notifications full-journey e2e — run on pve1 as root. Provisions a real dev VM, then walks
...
for cmd in curl jq pct; do
  command -v "$cmd" >/dev/null 2>&1 || { echo "missing required command: $cmd (run on pve1 as root)"; exit 2; }
done

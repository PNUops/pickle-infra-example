#!/usr/bin/env bash
# Applies the log-retention policy to pve1 and the pickle containers.
#
...
#      the (legacy) DB-backup log on pve1, and the dev mock-mail spool inside
...
# each root filesystem: pve1 75M/94G, 100 12M/7.8G, 101 56M/32G, 102 136M/7.8G.
...
apply_journald() { # $1 = cap, $2 = ctid ("" for pve1)
...
if [ "$DRY_RUN" = 0 ]; then
  ...
  if logrotate --debug /etc/logrotate.d/pickle >/dev/null; then
    ...
    echo "  [pve1] pickle: logrotate --debug rejected /etc/logrotate.d/pickle" >&2
    exit 1
  fi
  ...
  printf '  [pve1] '; journalctl --disk-usage

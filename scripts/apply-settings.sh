...
DEFAULTS=(
  ...
  "llm_usage_retention_days	0	LLM 원본 사용량 이벤트 보관 기간(일). 0이면 지우지 않습니다. 일별 집계는 이 설정과 무관하게 남지만, 지연 백분위와 시간대 분포처럼 원본을 읽는 통계는 이 기간까지만 답합니다. 켤 때의 하한은 90일입니다. 게이트웨이가 자기 스풀을 90일 보관하고 확인하지 못한 이벤트를 다시 보내므로, 그보다 먼저 잊으면 다시 온 이벤트를 새 것으로 세게 됩니다. (0 또는 90~3650)"

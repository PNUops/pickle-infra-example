#!/usr/bin/env bash
# Whole-system READ-ONLY health snapshot for pickle: pve1 host, LXC 100/101/102,
...
# 172.30.1.20) — so it must be probed FROM LXC 101, not the agent's own container
...
l22=$(pex 102 sh -c "ss -tln | grep -c '10.100.100.2:22'" 2>/dev/null)
if [ "${l22:-0}" -ge 1 ]; then rec "sshgw:listen-22" OK "10.100.100.2:22 (wg0)"
...
if ! pex 102 true >/dev/null 2>&1; then
  ...
  tws=$(timeout 10 curl -sk -o /dev/null -w '%{http_code}' \
        --resolve pickle.pusan.ac.kr:443:172.30.1.10 https://pickle.pusan.ac.kr/terminal/ws 2>/dev/null)
...
# Scope note: pve1 resolves the main domain through its own /etc/hosts entry

...
# HTTP-publishing e2e — run on pve1 as root. Provisions a real dev VM, starts a
# web server on it, publishes via the API, verifies HTTPS 200 via the origin :443
# SNI path (the Cloudflare leg is unreachable from pve1 — hairpin NAT), then
...
# The Cloudflare→pve1:443 leg is identical infra to opus (externally verified);
# from pve1 that leg can't be curled (hairpin NAT), so validate the NEW segment
...
if [ -n "$FQDN" ] && [ -n "$PIN" ]; then
  C=""
  for _ in $(seq 1 6); do
    C=$(curl -sS -o "$B" -w '%{http_code}' -k --pinnedpubkey "sha256//$PIN" --max-time 15 --resolve "$FQDN:443:172.30.1.10" "https://$FQDN/index.html" 2>/dev/null)
...
# pve1 — hairpin NAT — so `!=200` there would be vacuously true). -k on purpose:
...
if [ -n "$FQDN" ]; then
  : >"$B"
  C=$(curl -sS -o "$B" -w '%{http_code}' -k --max-time 15 --resolve "$FQDN:443:172.30.1.10" "https://$FQDN/index.html" 2>/dev/null)
  ...
  TLS_ERR=$(curl -sS -o /dev/null -k --max-time 15 \
       --resolve "$FQDN:443:172.30.1.10" "https://$FQDN/index.html" 2>&1 || true)
